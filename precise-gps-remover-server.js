...
class PreciseGPSRemover {
    ...
    // Precise EXIF data removal using exiftool
    ...
    // Smart OCR to detect GPS text only in corners
    async detectCornerText(imageBuffer) {
        try {
            console.log('🔍 Using smart OCR to detect GPS text in corners...');
            
            const image = await Jimp.read(imageBuffer);
            const width = image.getWidth();
            const height = image.getHeight();
            
            // Define corner regions (only where GPS text typically appears)
            const cornerRegions = [
                {
                    name: 'top-right',
                    x: width - (width * 0.4),
            ...
            for (const corner of cornerRegions) {
                try {
                    ...
                    const { data: { words } } = await Tesseract.recognize(cornerBuffer, 'eng+vie', {
                    ...
                    const gpsWords = words.filter(word => {
                    ...
                    if (gpsWords.length > 0) {
                        console.log(`🔍 Found ${gpsWords.length} GPS text in ${corner.name} corner`);
                        
                        // Convert corner coordinates back to full image coordinates
                        const cornerRegionsInFull = gpsWords.map(word => ({
                            x: corner.x + word.bbox.x0,
                            y: corner.y + word.bbox.y0,
                        ...
                        detectedRegions.push(...cornerRegionsInFull);
                    }
            ...
            console.log(`🔍 Total GPS text regions found: ${detectedRegions.length}`);
    ...
    // Precise text removal with smart inpainting
    async removeCornerText(imageBuffer, textRegions) {
        try {
            console.log(`🎨 Removing ${textRegions.length} GPS text regions with precise inpainting...`);
            ...
            for (const region of textRegions) {
                ...
                console.log(`  🎨 Removing "${region.text}" from ${region.corner} (${paddedX}, ${paddedY})`);
                
                // Smart inpainting for the text region
            ...
            console.log('✅ Precise GPS text removal completed');
            return await image.getBufferAsync(Jimp.MIME_JPEG);
            
        } catch (error) {
            console.error('❌ Precise text removal failed:', error);
            return imageBuffer;
        }
    }

    // Fallback: Target only corners with small black rectangles
    async applyCornerCoverage(imageBuffer) {
        try {
            console.log('🎯 Applying precise corner coverage for GPS text...');
            ...
            // Only cover corners where GPS text typically appears
            const svgRectangles = `
                <!-- Top-right corner (main GPS location) -->
                <rect x="${width - (width * 0.35)}" y="0" width="${width * 0.35}" height="${height * 0.25}" fill="black" />
                
                <!-- Bottom-right corner (alternative GPS location) -->
                <rect x="${width - (width * 0.45)}" y="${height - (height * 0.2)}" width="${width * 0.45}" height="${height * 0.2}" fill="black" />
                
                <!-- Top-left corner (date/time location) -->
                <rect x="0" y="0" width="${width * 0.25}" height="${height * 0.2}" fill="black" />
                
                <!-- Bottom-left corner (rare GPS location) -->
                <rect x="0" y="${height - (height * 0.15)}" width="${width * 0.25}" height="${height * 0.15}" fill="black" />
            `;
            ...
            console.log(`🎯 Applied 4 corner rectangles (only 25% of image area covered)`);
            ...
            console.error('❌ Corner coverage failed:', error);
            return imageBuffer;
        }
    }
    ...
    async addTextOverlay(imageBuffer, lat, lon, timeStr, action = 'add') {
        try {
            ...
            if (action === 'remove') {
                console.log('🎯 PRECISE GPS REMOVAL - Only targeting GPS text in corners...');
                
                // Step 1: Remove EXIF data with exiftool
                processedBuffer = await this.removeEXIFData(processedBuffer);
                
                // Step 2: Detect GPS text in corners only
                const cornerTextRegions = await this.detectCornerText(processedBuffer);
                
                if (cornerTextRegions.length > 0) {
                    console.log(`🎨 Found ${cornerTextRegions.length} GPS text regions, removing with precise inpainting...`);
                    processedBuffer = await this.removeCornerText(processedBuffer, cornerTextRegions);
                } else {
                    console.log('⚠️ No GPS text detected with OCR, applying corner coverage fallback...');
                    processedBuffer = await this.applyCornerCoverage(processedBuffer);
                }
                
                console.log('🎯 PRECISE GPS removal completed - Main image area preserved');
            ...
            console.error('❌ Error in precise processing:', error);
            throw error;
        }
    }
    ...
    async processImage(imageBuffer, filename, action, customGps = null) {
        try {
            ...
            return {
                ...
                status: action === 'add' ? 'Đã thêm GPS' : 'Đã xóa GPS cũ (Precise)',
...
app.post('/process', upload.array('images'), async (req, res) => {
    try {
        console.log('🎯 PRECISE Processing request started');
        ...
        console.log(`📁 Processing ${files.length} images with PRECISE tools`);
        ...
        const processor = new PreciseGPSRemover(config);
        const results = [];

        for (const file of files) {
            console.log(`\n🎯 Precise processing: ${file.originalname}`);
            const result = await processor.processImage(
                file.buffer, file.originalname, action,
                action === 'add' ? config.fallback_gps : null
            );
            results.push(result);
            console.log(`✅ Precise completed: ${file.originalname} - ${result.status}`);
        }
        ...
        console.log(`✅🎯 Precise processed ${results.length} images`);
        ...
        console.error('❌ Precise processing error:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});
...
// Start server
app.listen(PORT, () => {
    console.log(`🎯🎯🎯 PRECISE GPS REMOVER SERVER running at http://localhost:${PORT}`);
    console.log(`📁 Output directory: ${outputDir}`);
    console.log('🔗 ZIP export: http://localhost:' + PORT + '/export');
    console.log('🎯🎯🎯 PRECISE MODE ACTIVATED 🎯🎯🎯');
    console.log('📦 ZIP: Images only, no JSON');
    console.log('🔧 EXIF: ExifTool for complete metadata removal');
    console.log('🔍 OCR: Smart corner detection only');
    console.log('🎨 Coverage: Only 25% of image area (corners)');
    console.log('✅ Main image area: 75% preserved');
});
