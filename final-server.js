...
function cleanOutputDirectory() {
    try {
        if (fs.existsSync(outputDir)) {
            ...
            console.log(`🧹 Output directory cleaned. Deleted ${files.length} files.`);
        }
    } catch (error) {
...
class ImageProcessor {
    constructor(config = {}) {
        this.config = {
            ...
            rectangle_height_percent: 0.25, // Increased to 25% for better coverage
            ...config
        };
    }
    ...
    async addTextOverlay(imageBuffer, lat, lon, timeStr, action = 'add') {
        try {
            ...
            // Create SVG for overlay with larger, solid black rectangle
            const svg = `
                <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
                    <rect x="${rectX}" y="${rectY}" width="${width}" height="${rectHeight}" fill="black" />
                    ${svgText}
                </svg>
            `;
...
app.post('/process', upload.array('images'), async (req, res) => {
    try {
        ...
        // Clean output directory BEFORE processing new images
        ...
        const config = {
            ...
            font_size: parseInt(fontSize) || 28,
            fallback_gps: {
        ...
        console.log(`📊 Final output contains ${finalFiles.length} files:`, finalFiles);
...
app.get('/export', async (req, res) => {
    try {
        ...
        const logData = {
            ...
            imageList: files,
            processingSettings: {
                rectangle_height_percent: 0.25,
                font_size: 28,
                coverage: 'Enhanced to hide old GPS text'
            }
        };
...
app.listen(PORT, () => {
    ...
    console.log('🧹 Auto-cleaning enabled for each batch');
});
