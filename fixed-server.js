...
class ImageProcessor {
    constructor(config = {}) {
        this.config = {
            ...
            font_size: 24,
            padding: 10,
            rectangle_height_percent: 0.2, // Increased to 20% to better cover old text
            ...config
        };
    }
    ...
    async addTextOverlay(imageBuffer, lat, lon, timeStr, action = 'add') {
        try {
            ...
            // Create SVG for overlay with larger, more opaque rectangle
            const svg = `
                <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
                    <rect x="${rectX}" y="${rectY}" width="${width}" height="${rectHeight}" fill="${this.config.rectangle_color}" opacity="1.0" />
                    ${svgText}
                </svg>
            `;
