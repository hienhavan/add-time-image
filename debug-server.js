...
// DEBUG: Force clean with detailed logging
function debugCleanOutputDirectory() {
    console.log('\n🔍🔍🔍 DEBUG CLEAN START 🔍🔍🔍');
    ...
    try {
        if (fs.existsSync(outputDir)) {
            ...
            console.log(`📁 Files found before clean: ${files.length}`);
            ...
            // Delete each file with detailed logging
            console.log('\n🗑️ Deleting files...');
            files.forEach((file, index) => {
                const filePath = path.join(outputDir, file);
                try {
            ...
            console.log(`📁 Files remaining after clean: ${remainingFiles.length}`);
            remainingFiles.forEach((file, index) => {
                console.log(`  ${index + 1}. ${file}`);
            });
            
            if (remainingFiles.length === 0) {
                console.log('✅✅✅ DIRECTORY COMPLETELY CLEAN! ✅✅✅');
            } else {
                console.log('❌❌❌ DIRECTORY NOT CLEAN - FILES REMAIN! ❌❌❌');
            }
        } else {
            console.log('📁 Output directory does not exist');
        }
    } catch (error) {
        console.error('❌ ERROR in debug clean:', error);
    }
    
    console.log('🔍🔍🔍 DEBUG CLEAN END 🔍🔍🔍\n');
}
...
app.post('/process', upload.array('images'), async (req, res) => {
    try {
        console.log('\n🚀🚀🚀 NEW PROCESSING REQUEST STARTED 🚀🚀🚀');
        ...
        // CLEAN FIRST - with detailed logging
        console.log('\n🧹🧹🧹 CLEANING BEFORE PROCESSING 🧹🧹🧹');
        debugCleanOutputDirectory();

        console.log(`\n📁📁📁 PROCESSING ${files.length} NEW IMAGES 📁📁📁`);
        ...
        console.log(`📁 Final files in output directory: ${finalFiles.length}`);
        ...
        console.log('🚀🚀🚀 PROCESSING REQUEST COMPLETED 🚀🚀🚀\n');
...
app.post('/clean', (req, res) => {
    try {
        ...
        debugCleanOutputDirectory();
        res.json({ success: true, message: 'Output directory debug cleaned' });
    } catch (error) {
...
app.listen(PORT, () => {
    ...
    console.log('🔍🔍🔍 DETAILED DEBUGGING ENABLED 🔍🔍🔍');
