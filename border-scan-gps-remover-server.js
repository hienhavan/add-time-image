...
class BorderScanGPSRemover {
    ...
    // Scan 30% border areas for text and numbers
    async scanBorderAreas(imageBuffer) {
        try {
            console.log('🔍 Scanning 30% border areas for ALL text and numbers...');
            ...
            for (const border of borderAreas) {
                try {
                    console.log(`🔍 Scanning ${border.name} (${border.width}x${border.height})...`);
                    ...
                    // Use OCR to detect ALL text and numbers
                    ...
                    console.log(`🔍 Found ${words.length} text elements in ${border.name}`);
                    
                    // Convert border coordinates back to full image coordinates
                    const borderRegionsInFull = words.map(word => ({
                    ...
                    allTextRegions.push(...borderRegionsInFull);
            ...
            console.error('❌ Border scanning failed:', error);
            return [];
        }
    }

    // Remove all text from border areas
    async removeBorderText(imageBuffer, textRegions) {
        try {
            console.log(`🎨 Removing ALL text from border areas (${textRegions.length} regions)...`);
            ...
            for (const region of textRegions) {
                ...
                console.log(`  🎨 Removing "${region.text}" from ${region.border} (${paddedX}, ${paddedY})`);
                
                // Apply black overlay to text region (more aggressive for borders)
            ...
            console.log('✅ Border text removal completed');
            return await image.getBufferAsync(Jimp.MIME_JPEG);
            
        } catch (error) {
            console.error('❌ Border text removal failed:', error);
            return imageBuffer;
        }
    }

    // Alternative: Apply complete black coverage to 30% border areas
    async applyBorderCoverage(imageBuffer) {
        try {
            console.log('🖤 Applying complete black coverage to 30% border areas...');
            ...
            // Create black coverage for all 4 borders
            const svgRectangles = `
                <!-- Top border (30%) -->
                <rect x="0" y="0" width="${width}" height="${height * borderPercent}" fill="black" />
                
                <!-- Bottom border (30%) -->
                <rect x="0" y="${height - (height * borderPercent)}" width="${width}" height="${height * borderPercent}" fill="black" />
                
                <!-- Left border (30%) -->
                <rect x="0" y="0" width="${width * borderPercent}" height="${height}" fill="black" />
                
                <!-- Right border (30%) -->
                <rect x="${width - (width * borderPercent)}" y="0" width="${width * borderPercent}" height="${height}" fill="black" />
            `;
            ...
            console.log(`🖤 Applied 30% black coverage to all 4 borders`);
            console.log(`🖤 Center area preserved: ${(1 - borderPercent * 2) * 100}% of image`);
            ...
            console.error('❌ Border coverage failed:', error);
            return imageBuffer;
        }
    }
    ...
    async addTextOverlay(imageBuffer, lat, lon, timeStr, action = 'add') {
        try {
            ...
            if (action === 'remove') {
                console.log('🔍🖤 BORDER SCAN GPS REMOVAL - Scanning 30% borders for ALL text...');
                
                // Step 1: Remove EXIF data
                processedBuffer = await this.removeEXIFData(processedBuffer);
                
                // Step 2: Scan 30% border areas for text
                const borderTextRegions = await this.scanBorderAreas(processedBuffer);
                
                if (borderTextRegions.length > 0) {
                    console.log(`🎨 Found ${borderTextRegions.length} text regions, removing from borders...`);
                    processedBuffer = await this.removeBorderText(processedBuffer, borderTextRegions);
                } else {
                    console.log('⚠️ No text detected in borders, applying complete border coverage...');
                    processedBuffer = await this.applyBorderCoverage(processedBuffer);
                }
                
                console.log('🔍🖤 BORDER SCAN GPS removal completed');
            ...
            console.error('❌ Error in border scan processing:', error);
            throw error;
        }
    }
    ...
    async processImage(imageBuffer, filename, action, customGps = null) {
        try {
            ...
            return {
                ...
                status: action === 'add' ? 'Đã thêm GPS' : 'Đã xóa GPS cũ (Border Scan)',
...
app.post('/process', upload.array('images'), async (req, res) => {
    try {
        console.log('🔍🖤 BORDER SCAN Processing request started');
        ...
        console.log(`📁 Processing ${files.length} images with BORDER SCAN tools`);
        ...
        const processor = new BorderScanGPSRemover(config);
        const results = [];

        for (const file of files) {
            console.log(`\n🔍🖤 Border scan processing: ${file.originalname}`);
            const result = await processor.processImage(
                file.buffer, file.originalname, action,
                action === 'add' ? config.fallback_gps : null
            );
            results.push(result);
            console.log(`✅ Border scan completed: ${file.originalname} - ${result.status}`);
        }
        ...
        console.log(`✅🔍🖤 Border scan processed ${results.length} images`);
        ...
        console.error('❌ Border scan processing error:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});
...
// Start server
app.listen(PORT, () => {
    console.log(`🔍🖤🔍🖤 BORDER SCAN GPS REMOVER SERVER running at http://localhost:${PORT}`);
    console.log(`📁 Output directory: ${outputDir}`);
    console.log('🔗 ZIP export: http://localhost:' + PORT + '/export');
    console.log('🔍🖤🔍🖤 BORDER SCAN MODE ACTIVATED 🔍🖤🔍🖤');
    console.log('📦 ZIP: Images only, no JSON');
    console.log('🔧 EXIF: ExifTool for complete metadata removal');
    console.log('🔍 Scan: 30% border areas on all 4 sides');
    console.log('🖤 Coverage: ALL text and numbers in borders');
    console.log('✅ Center area: 40% preserved (30% borders each side)');
});
